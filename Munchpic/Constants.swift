//
//  Constants.swift
//  Munchpic
//
//  Created by Pavithra Jayanna on 28/05/17.
//  Copyright © 2017 Pavithra Jayanna. All rights reserved.
//

import Foundation
class Constants{
    
    static let kBaseUrl : String = "http://ekalavyatech.com/munchpic.com/munchpicPHP/"
    

    static let kGetPost : String = "getPosts.php?"
    static let kGetCollections : String = "getCollections.php"
    static let kGetIngredients : String = "getIngredients.php"
    static let kGetComments : String = "getComments.php"
    static let kGetCornivals : String = "getCornivals.php"
    static let kGetCollectionCategories : String = "getCollectionCategories.php"
    static let kGetLoves : String = "getLoves.php"
    static let kGetParticularPost : String = "getParticularPost.php"
    static let kGetTrends : String = "getTrends.php"
    static let kGetUserPosts : String = "getUserPosts.php"

    static let kInsertLoves : String = "insertLoves.php"
    static let kInsertCollections : String = "insertCollections.php"
    static let kInsertComments : String = "insertComments.php"
    static let kInsertPost : String = "insertPost.php"
    
    static let kDeleteCollection : String = "deleteCollection.php"


    static let KForgotPassword : String = "forgotPassword.php"
    static let KLogin : String = "login.php?"
    static let KResetPassword : String = "resetPassword.php"
    static let KRegister : String = "register.php"
    static let KActivation : String = "activation.php"
    
    static let KUpdateUserInfo : String = "updateUserInfo.php"
    static let KUpdateUserPost : String = "updateUserPost.php"
    static let KGetUserInfo : String = "getUserInfo.php?"
    
    static let KFilter : String = "filter.php?"
    
}
